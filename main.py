...
def game_over_text():
    game_over_text = game_over_font.render("THE DEFENCE HAS BEEN BROKEN!!", True, (255, 255, 255))
    gameDisplay.blit(game_over_text, (100, 300))
    game_over_text1 = game_over_font.render("GAME OVER", True, (255, 255, 255))
    gameDisplay.blit(game_over_text1, (300, 350))
    ...
    def update(self, board, cash, Images, oldMode, Diff):
        ...
        if not oldMode:
            ...
            for j in range(4):
                for i in range(2):
                    if self.step == 0:
                        if 660 + i * 100 <= pos[0] <= 660 + i * 100 + 60 and 150 + j * 70 <= pos[
                            1] <= 150 + j * 70 + 60 and pressed[0] == 1 and self.selecting == False:
                            self.selecting = True
            ...
            if self.selected != 0:
                ...
                gameDisplay.blit(font_20.render(
                    "Cost: " + str(int(self.Costs[self.selected - 1] * Multiplier[Difficulties.index(Diff)])), True,
                    (0, 0, 0)), (690, 450))
            ...
            if self.selecting == True and cash >= self.Costs[self.selected - 1] * Multiplier[Difficulties.index(Diff)]:
                if self.selected == 1:
                    ...
                    gameDisplay.blit(pygame.transform.scale(Images["Flamethrower"], (25, 35)),
                                     (pos[0] - 15, pos[1] - 15))
                    ...
                    gameDisplay.blit(pygame.transform.scale(Images["GlaiveSpinner"], (40, 20)),
                                     (pos[0] - 20, pos[1] - 10))
            ...
            if self.selected != 0 and 0 <= pos[0] <= 640 and 0 <= pos[1] <= 480 and pressed[
                0] == 0 and self.selecting == True:
                if keys[303] != 1 and keys[304] != 1:
                    self.selecting = False
                if board[int(pos[1] / 40)][int(pos[0] / 40)] == 0 and cash >= self.Costs[self.selected - 1] * \
                        Multiplier[Difficulties.index(Diff)]:
                    board[int(pos[1] / 40)][int(pos[0] / 40)] = tower.Tower(int(pos[0] / 40) * 40 + 5,
                                                                            int(pos[1] / 40) * 40 + 5, self.selected,
                                                                            Diff)
            ...
            if self.selected != 0:
                gameDisplay.blit(font_20.render(str(self.names[self.selected - 1]), True, (0, 0, 0)), (
                    740 - int(font_20.size(str(self.names[self.selected - 1]))[0] / 2), 425))
                gameDisplay.blit(font_20.render("Cost: " + str(self.Costs[self.selected - 1]), True, (0, 0, 0)),
                                 (690, 450))

            # Buying a new tower
            if self.cooldown <= 0 and self.selected != 0 and 0 <= pos[0] <= 640 and 0 <= pos[1] <= 480 and pressed[
                0] == 1:
                if board[int(pos[1] / 40)][int(pos[0] / 40)] == 0 and cash >= self.Costs[self.selected - 1]:
                    board[int(pos[1] / 40)][int(pos[0] / 40)] = tower.Tower(int(pos[0] / 40) * 40 + 5,
                                                                            int(pos[1] / 40) * 40 + 5, self.selected)
...
class Start():
    ...
    def update(self):
        ...
        if ((pressed[0] == 1 and self.x <= pos[0] <= self.x + self.width and self.y <= pos[
            1] <= self.y + self.height) or (keys[32] == 1)) and not self.changed:
            self.changed = True
            self.speed += 1
            if self.speed > 2:
                self.speed = 1

        if not ((pressed[0] == 1 and self.x <= pos[0] <= self.x + self.width and self.y <= pos[
            1] <= self.y + self.height) or (keys[32] == 1)):
            self.changed = False
...
class AutoPlay():
    ...
    def update(self):
        ...
        if self.x <= pos[0] <= self.x + self.width and self.y <= pos[1] <= self.y + self.height and pressed[
            0] == 1 and self.cooldown == 0:
...
def game_loop(load, mapName, Diff=""):
    ...
    while game_run:
                ...
                if monster[0].dead:
                    ...
                    if len(monster[0].addMonster) != 0:
                        for i, mons in enumerate(monster[0].addMonster):
                            tempMonster = [monsters.Monster(mons, wave, monster[0].camo, Images, mapName),
                                           monster[1] - 30 * (i + 1)]
                            tempMonster[0].x, tempMonster[0].y = monster[0].x, monster[0].y
                            tempMonster[0].step = monster[0].step
                            tempMonster[0].cooldown = 5
            ...
            for j, row in enumerate(Board):
                for i, tile in enumerate(row):
                    ...
                    try:
                        ...
                        data += "2" + "#"
                        for item in [tile.rank, tile.pops, tile.score, tile.pierce, tile.damage, tile.speed,
                                     tile.shotAmount, tile.size, tile.path, tile.seeking,
                                     tile.bulletSpeed, tile.camo, tile.dead, tile.fireDamage, tile.fireLength,
                                     tile.fireLasting, tile.Permaslow, tile.slowAmount,
                                     tile.ExplodeTime, tile.bombRange, tile.crateValue, tile.autoCollect,
                                     tile.expireTime, tile.glaiveCount, tile.glaiveSpeed,
        ...
        ColorBoard = [[(200, 0, 0), (0, 0, 200)], [(200, 200, 0), (0, 200, 200)], [(100, 100, 100), (0, 200, 0)],
                      [(200, 200, 200), (100, 150, 200)]]
        if selection.step == 0:
            ...
            for i in range(2):
                if selection.step == 0:
                    ...
                    if [i, j] not in [[0, 1], [1, 0], [1, 1], [0, 3]]:
                        pygame.draw.rect(gameDisplay, ColorBoard[j][i], (660 + i * 100 + 10, 150 + j * 70 + 10, 40, 40),
                                         0)
                    elif j == 0 and i == 1:
                        gameDisplay.blit(pygame.transform.scale(Images["NinjaBase"], (30, 30)),
                                         (660 + i * 100 + 15, 150 + j * 70 + 20))
                        gameDisplay.blit(pygame.transform.scale(Images["NinjaGun"], (30, 40)),
                                         (660 + i * 100 + 15, 150 + j * 70 + 5))
                    elif j == 1 and i == 1:
                        gameDisplay.blit(pygame.transform.scale(Images["IceTower"], (30, 30)),
                                         (660 + i * 100 + 15, 150 + j * 70 + 20))
                    elif j == 3 and i == 0:
                        gameDisplay.blit(pygame.transform.scale(Images["GlaiveBase"], (35, 35)),
                                         (660 + i * 100 + 12, 150 + j * 70 + 12))
                        gameDisplay.blit(pygame.transform.scale(Images["GlaiveSpinner"], (45, 20)),
                                         (660 + i * 100 + 7, 150 + j * 70 + 10 + 10))
                        gameDisplay.blit(pygame.transform.scale(Images["GlaiveTop"], (13, 13)),
                                         (660 + i * 100 + 10 + 13, 150 + j * 70 + 10 + 13))
                    else:
                        gameDisplay.blit(pygame.transform.scale(Images["Flamethrower"], (40, 60)),
                                         (660 + i * 100 + 10, 150 + j * 70))
                elif selection.step == 1:
                    if (i, j) == (0, 0):
                        pygame.draw.rect(gameDisplay, (140, 90, 40), (660 + i * 100, 150 + j * 70, 60, 60), 0)
                        pygame.draw.rect(gameDisplay, (210, 180, 140), (660 + i * 100, 150 + j * 70, 60, 60), 3)
                        gameDisplay.blit(pygame.transform.scale(Images["Cannon"], (30, 45)),
                                         (660 + i * 100 + 15, 160 + j * 70))
        ...
        for j, row in enumerate(Board):
            for i, tile in enumerate(row):
                try:
                    ...
                    if tile.x <= pos[0] <= tile.x + tile.width and tile.y <= pos[1] <= tile.y + tile.height and pressed[
                        0] == True:
        ...
        for j, row in enumerate(Board):
            for i, tile in enumerate(row):
                try:
                    ...
                    if tile.selected:
                        pygame.draw.circle(gameDisplay, (0, 0, 0, 125),
                                           (tile.x + int(tile.width / 2), tile.y + int(tile.height / 2)),
                                           int(tile.range), 2)
